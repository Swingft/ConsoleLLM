...
class BaseAnalyzer:
    ...
    def load_file_cache(self, cache_path: str) -> Dict[str, Dict[str, Any]]:
        """파일 캐시 로드"""
    ...
    def save_file_cache(self, cache: Dict[str, Dict[str, Any]], cache_path: str):
        """파일 캐시 저장"""
        try:
    ...
    def run_swift_analyzer(self, source_file_path: str, analyzer_path: Optional[str] = None) -> Optional[str]:
        """
        ...
        try:
            command_str = f'"{analyzer_path}" "{source_file_path}"'
            ...
            if process.returncode != 0:
                ...
                print(f"Warning: AST analyzer failed for {source_file_path}. Error: {error_message}")
                return None
            ...
            print(f"Warning: AST analysis timed out for {source_file_path}")
            return None
        except Exception as e:
            print(f"Warning: AST analysis failed for {source_file_path}: {e}")
            return None
    ...
    def find_source_files_with_identifiers(self, project_path: str, identifiers: List[str]) -> List[str]:
        """프로젝트에서 특정 식별자를 포함한 Swift/Objective-C 파일들을 찾음"""
        matching_files = []
        # Swift와 Objective-C 헤더 파일 모두 검색
        source_files = []
        source_files.extend(glob.glob(os.path.join(project_path, "**/*.swift"), recursive=True))
        source_files.extend(glob.glob(os.path.join(project_path, "**/*.h"), recursive=True))

        print(f"Scanning {len(source_files)} source files for identifiers: {identifiers}")
    ...
    def get_all_source_files(self, project_path: str) -> List[str]:
        """프로젝트의 모든 Swift 및 Objective-C 헤더 파일들을 찾음"""
        source_files = []
        ...
        source_files.extend(swift_files)
        source_files.extend(h_files)
        print(
            f"Found {len(source_files)} source files in project ({len(swift_files)} Swift, {len(h_files)} Objective-C headers)")
        return source_files

    def get_all_swift_files(self, project_path: str) -> List[str]:
        """프로젝트의 모든 Swift 파일들을 찾음 (하위 호환성 유지)"""
        swift_files = glob.glob(os.path.join(project_path, "**/*.swift"), recursive=True)
    ...
    def generate_analysis(self, source_file_path: str) -> Dict[str, Any]:
        """
        단일 소스 파일에 대한 분석 수행 (하위 클래스에서 구현)
        ...
        system_prompt, user_prompt = self.create_model_input(source_file_path, ast_json)
    ...
    def analyze_project(self, project_path: str = None, config_path: str = None,
        ...
        Args:
            project_path: 프로젝트 디렉토리 경로 (우선순위 높음)
